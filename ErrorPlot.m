close all
clear all 
% Ce script test la matrice de Projection sur des points dans le repere monde et caclules les erreurs
...
% Points dans le repere monde 
x2= [0;6.8;6.8;0]*10^(-2);
y2 = [0;0;14;14]*10^(-2);
K= GetCameraParams(calibrationSession);
P = Projection_matrix(x2,y2,x,y,K);
P_noNorm = Projection_matrix_noNomr(x2,y2,x,y,K);
imshow(img)
axis on
hold on
...
% Decommenter les deux prochaines lognes pour tracé les points dans le repere images 
